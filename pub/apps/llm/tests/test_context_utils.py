...
class GetContextTests(TestCase):
    class DummyExchange:
        def __init__(self, query, raw_answer, attachments=None):
            self.query = query
            self.raw_answer = raw_answer
    ...
    class DummyExchangesQueryset:
        ...
        def __getitem__(self, item):
            return self._exchanges[item]

    class DummyConversation:
        def __init__(self, exchanges):
            self.exchanges = GetContextTests.DummyExchangesQueryset(exchanges)

    def test_get_context_no_conversation(self):
        result = context.get_context(None)
